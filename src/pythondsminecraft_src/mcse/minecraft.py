...
""" Minecraft for serveur v1.16.1 and later
    ...
    @ modification S. PINET, LPO ROUVIERE TOULON - version 3.0 - jul 2020
    """

""" Updated to include functionality provided by RaspberryJuice and more :
- getBlocks()
- getDirection()
- getPitch()
- getRotation()
...
- setFixed()
- setFurnace()
- setGrindstone() serveur 1.16.1
- setWall() serveur 1.16.1
...
class CmdPlayer(CmdPositioner):
    """Methods for the host (server) player"""
    def __init__(self, connection):
    ...
    def removeEntities(self, typeEntity,distance=10):
        """Remove entities all entities near entity (distanceFromPlayerInBlocks:int, typeEntity: str ) => (removedEntitiesCount:int)"""
        """If distanceFromPlayerInBlocks:int is not specified then default 10 blocks will be used"""
        return int(self.conn.sendReceive(b"player.removeEntities", distance, typeEntity))
...
class Minecraft:
    ...
    def setBlockDir(self, *args):
        """Place un bloc de type BlockData: Directional (x,y,z,material,direction -  v 1.15.1
    ...
    def setBlockDistrib(self, *args):
        """Place un distributeur blockData : Dispenser (x,y,z, Material, Face, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : DISPENSER, DROPPER

        Face (string) : NORTH, SOUTH, EAST, WEST

        Activé (booléen) : True ou False (défaut)
        ...
        self.conn.send(b"world.setBlockDistrib",flatargs[0:nb]) 
    ...
    def setBlockPower(self, *args):
        """Place un block de puissance blockData : Powerable (x,y,z, Material, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : ACACIA_PRESSURE_PLATE, BIRCH_PRESSURE_PLATE, CRIMSON_PRESSURE_PLATE, DARK_OAK_PRESSURE_PLATE, JUNGLE_PRESSURE_PLATE, OAK_PRESSURE_PLATE, POLISHED_BLACKSTONE_PRESSURE_PLATE,  SPRUCE_PRESSURE_PLATE, STONE_PRESSURE_PLATE, WARPED_PRESSURE_PLATE.
        ...
        self.conn.send(b"world.setBlockPower",flatargs[0:nb])
    ...
    def setBlockWithBlocData(self, *args):
        """Set block (x,y,z,material,data -  v 1.15.1 -  july 2020
        
        Material must be one of this at :
        https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/Material.html

        data is a BlockData and must have the RIGHT configuration for example : 
                    "data = "minecraft:chest[waterlogged=true]"
        difficult to achieve ... and to use as, for example, copy block from a place to another ...
        
        """
        intFloor(args[0:2])
        # Attention : on remplace les ',' de séparation dans la partie BlockData 
        # par des '/' pour éviter la confusion de séparateur par la 
        # fonction java "remote.session.java handleLine(String line)" entre la séparation des
        # arguments principaux (X, Y, Z, Material, BlockDAta) et le contenu du BlockData
        # la fonction java setBlockWithBlocData est modifiée pour les supprimer
        param = list(args)        
        chaine = param[4].replace(',','/')       
        param[4]=chaine

        args = tuple(param)
        # print(args)
        self.conn.send(b"world.setBlockWithBlocData", args)
    ...
    def setEntonnoir(self, *args):
        """Place un entonnoir (x,y,z, Material, Face, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : HOOPER 

        Face (string) : DOWN (défaut)

        Activé (booléen) : True (défaut) ou False
        ...
        self.conn.send(b"world.setEntonnoir",flatargs[0:nb])        
    ...
    def setGrindstone(self, *args):
        """Place un bloc de type BlockData: Grindstone (x,y,z,Material,face,faceAttachée)

        x,y,z (int) : position en X, Y, Z dans le monde
        
        Place un bloc de type BlocData Grindstone (voir : 
            https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/Material.html)
        GRINDSTONRE

        face  : NORTH, SOUTH, EAST, WEST  - valeur par défaut NORTH
        
        faceAttachée : CEILING, FLOOR, WALL
        """
        ...
        intFloor(flatargs[0:3])
        #print(flatargs)
        self.conn.send(b"world.setGrindstone",flatargs[0:nb])

    def setObserver(self, *args):
        """Place un observateur   BlockData : Observer (x,y,z, Material, Face, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : OBSERVER 
        ...
        self.conn.send(b"world.setObserver",flatargs[0:nb]) 
    ...
    def setRail(self, *args):
        """Place un bloc rail de type rail  BlockData : RedstoneRail (x,y,z, Material, forme, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : ACTIVATOR_RAIL, DETECTOR_RAIL,  POWERED_RAIL, 

        forme (string) : ASCENDING_EAST, ASCENDING_NORTH, ASCENDING_SOUTH, ASCENDING_WEST, EAST_WEST, NORTH_EAST, NORTH_SOUTH, NORTH_WEST, SOUTH_EAST, SOUTH_WEST
        ...
        self.conn.send(b"world.setRail",flatargs[0:nb])        
    ...
    def setSwitch(self, *args):
        """Place un bloc rail de type interrupteur  BlockData : Switch (x,y,z, Material, Face, FaceAttache, Activé)

        x,y,z (int) : position en X, Y, Z dans le monde

        Material (string) : ACACIA_BUTTON,  BIRCH_BUTTON, CRIMSON_BUTTON, DARK_OAK_BUTTON, JUNGLE_BUTTON, LEVER, OAK_BUTTON, POLISHED_BLACKSTONE_BUTTON, SPRUCE_BUTTON, STONE_BUTTON, WARPED_BUTTON, 
        ...
        self.conn.send(b"world.setSwitch",flatargs[0:nb])         
    ...
    def setWall(self, *args):
        """from 1.16.1 version
        Set a Wall - mur bloc de type BlockData : wall càd
        bloc avec une propriété de hauteur (LOW, NONE, TALL) pour chaque
        face (NORTH, SOUTH, EAST, WEST) - La face UP est TRUE or FALSE
        (x,y,z, Material, face,Hauteur)
        bloc avec propriété Direction et Hauteur (x,y,z, Material, [face*,hauteur*])

        x,y,z (int) : position en X, Y, Z dans le monde
        
        face : EAST, NORTH, SOUTH, UP, WEST
        
        Hauteur : LOW (mur bas), NONE(pas de mur un pilier), TALL (mur haut)

        Material (string) :  ANDESITE_WALL, BLACKSTONE_WALL,
        BRICK_WALL, COBBLESTONE_WALL, DIORITE_WALL,      
        END_STONE_BRICK_WALL, GRANITE_WALL,
        MOSSY_COBBLESTONE_WALL, MOSSY_STONE_BRICK_WALL,
        NETHER_BRICK_WALL, POLISHED_BLACKSTONE_BRICK_WALL, POLISHED_BLACKSTONE_WALL, PRISMARINE_WALL, RED_NETHER_BRICK_WALL,
        ...
        self.conn.send(b"world.setWall",flatargs[0:nb])         
